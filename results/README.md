# Results — start here

> ℹ️ **Click a `.md` file to read it here on GitHub.** GitHub renders Markdown but cannot preview
> PDFs or spreadsheets, so those show an "unable to render" message — use the download button.
> The content is identical.

**`FILE-INDEX.md` (or `pdf/FILE-INDEX.pdf`) is the full table of every file and what it contains.**

**👉 `THE-COMPLETE-GUIDE.pdf` (in `pdf/`) explains everything. `MASTER-all-opportunities.xlsx` is
where you filter and decide.**

| File | What it is |
|---|---|
| **THE-COMPLETE-GUIDE.md / pdf** | **The single document: 1,029 opportunities, 56 countries, how to read them, what to do next** |
| **MASTER-all-opportunities.xlsx** | **Everything in one filterable workbook.** Tabs: ★ Best Bets · Strong + English/French · Fully funded · Has a scholarship · Scholarship index · Free or cheap · Everything · one per region |
| 🗺️ **[pdf/SPAIN-ROADMAP.pdf](./pdf/SPAIN-ROADMAP.pdf)** *(download)* | **The dated plan: start at Estudio Sampere on Monday 31 January 2028.** A 24-week course, B2 earned in Alicante, no flight home, master's in September 2028 — with the reasoning for every date, the one weak joint in the plan, and what to do now |
| **ROUTES-AND-TIMELINE.md** | **The four routes to a master's, with dates.** Spain has no official degree in music production or sound design; the one Spanish option that fits is taught in English; nothing funds a language year. What each route costs in money and in time, and which decisions are actually urgent |
| **SPAIN-COST-OF-LIVING.md** | **Fifteen Spanish university cities ranked by what a student actually spends** — room, food, transport, utilities, going out. Plus the non-EU master's tuition surcharge by region: four regions charge you the same as a Spanish student, Madrid can charge €10,153/yr |
| EUROPE-GAPFILL.md / pdf / xlsx | The 85 institutions the earlier searches missed — public/private, language, non-EU fee |
| PRIVATE-SCHOOLS.md / pdf / xlsx | Private institutions in Spain, Portugal, Netherlands, Italy — accreditation first |
| SOUND-DESIGN-EUROPE.md / xlsx | The 319-programme sound-design and production deep dive |
| SHORTLIST.md | The earlier ranked shortlist with study locations |
| ACTION-TIMELINE.md | Month-by-month plan to September 2027 |
| PATHS-DEEP-DIVE.md | The three career paths explained, and what a portfolio means |
| COULD-NOT-VERIFY.md | Everything that could not be confirmed, listed honestly |
| opportunities.xlsx | The original worldwide sweep dataset |

All PDFs are in `pdf/`.
